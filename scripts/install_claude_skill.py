#!/usr/bin/env python3
"""Install the make-figure skill into Claude Code."""
...
def copy_skill(src: Path, dest: Path, *, dry_run: bool, force: bool) -> None:
    if dry_run:
        print(f"Would install Claude Code skill from {src} to {dest}")
        return
...
def main() -> int:
    ...
    if missing:
        print("Claude Code package is incomplete. Missing:", file=sys.stderr)
        for path in missing:
    ...
    if args.validate_only:
        print(f"Claude Code package is valid: {src}")
